...
(defmacro do-cmd [call]
  `(try
     (let [id# ~call]
       ...
       (marshal 200 {:response "ok" :stat (stat *world* id#)}))
     (catch Exception ~'e
...
(defroutes api-routes
  ...
  (GET "/_admin_/feed" [] (marshal 200 (get-feed *world*)))
  (GET "/_admin_/place-food/:x/:y" {{x :x y :y} :params} (marshal 200 (place-food *world* [(Integer/parseInt x) (Integer/parseInt y)])))
  (GET "/_admin_/remove-food/:x/:y" {{x :x y :y} :params} (marshal 200 (remove-food *world* [(Integer/parseInt x) (Integer/parseInt y)])))
