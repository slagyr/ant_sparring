(ns ants.api.core
  (:require [ants.api.app :as app]
            [ants.api.remote :as remote]
            [ants.engine.core :as ant]
            [ants.view.arena :as arena]
...
(defn refreshable [handler-sym]
  (if (env/development?)
    (fn [request] (@(resolve-var handler-sym) request))
    (resolve-var handler-sym)))

(defroutes web-routes
  (refreshable 'ants.api.remote/handler)
  (refreshable 'ants.api.core/handler)
...
(defn -main []
  (alter-var-root #'app/app remote/start)
